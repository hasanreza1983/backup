...
module.exports = {
    ...
    assignCrmCampaignsToModel: async (obj, args, context, info) => {
        ...
        if (arrErrors.error != null) {
            ...
            try {
                ...
                let arrObj = [];
                args.input.Campaigns.map((campaignId, index) => {
                    let objRow = {};
                ...
                const objCampaignAssignment = await model.CampaignLink.bulkCreate(arrObj);
    ...
    unAssignCrmCampaignsToModel: async (obj, args, context, info) => {
        arrErrors = commonValidation.validateCampaignUnAssignmentInput(args.input); // validation for Lead input data
        //arrErrors.error = null;
        if (arrErrors.error != null) {
            throw new Error(arrErrors.error.details[0].message);
        } else {
            try {
                ...
                const objCampaignUnAssignment = await model.CampaignLink.findOne({
                    where: {
                        id: args.input.id,
                ...
                if (objCampaignUnAssignment) {
                    let isCampaignUnAssignment = await model.CampaignLink.destroy({
                        where: {
                            id: args.input.id
                        }
                    });

                    if (isCampaignUnAssignment) {
                        objCampaignUnAssignment.message = "Campaigns unassigned successfully!";
                    }
                }
                return objCampaignUnAssignment;
    ...
    updateCrmCampaignStatusForModel: async (obj, args, context, info) => {
        arrErrors = commonValidation.validateCampaignAssignmentUpdateInput(args.input); // validation for Lead input data
        //arrErrors.error = null;
        if (arrErrors.error != null) {
            throw new Error(arrErrors.error.details[0].message);
        } else {
            try {
                const objCampaignAssignmentAlter = await model.CampaignLink.findOne({
                    where: {
                        id: args.input.id,
                        model_name: args.input.model_name,
                        model_id: args.input.model_id
                ...
                if (objCampaignAssignmentAlter) {
                    ...
                    if (isCampaignAssignmentUpdated) {
                        objCampaignAssignmentAlter.message = "Campaign member status updated successfully!";
                    }
                }
                return objCampaignAssignmentAlter;
