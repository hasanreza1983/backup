/*
* This validation file is for type CrmCompanyNote resolver
* Make Schema to validate CrmCompanyNote User Inputs
...
let schemaCreateInput = Joi.object().keys({
	...
	CompanyNoteAttachements: Joi.array().items(Joi.object().keys({
...
let schemaUpdateInput = Joi.object().keys({
	...
	CompanyNoteAttachements: Joi.array().items(Joi.object().keys({
		...
		id_crm_company_note: Joi.number().integer(),
		minio_file_id: Joi.string(),
